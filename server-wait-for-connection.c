...
int main(int argc, char *argv[])
{
    ...
    printf("Server: Wait for Connection - Ctrl-C to kill.\n");
    listen(sockfd,5);
    struct sockaddr_in cli_addr;            // Clent internet address
    int clilen = sizeof(cli_addr);          // Size of address
    int newsockfd = accept(                 // Wait for client connection
        sockfd,
...
/*
# This: server-wait-for-connection.c
# Prev: server-bind-to-port.c
# Next: server-send-hello-world.c

# Build & run:
gcc server-wait-for-connection.c -o server && ./server 9001
*/
